let CONFIG = [{"bullets": 8}]


let TARGETS = [{"boar":[300, 0, 200]},
           {"vulture":[50, 25, -100]},
           {"coyote":[70, 0, 0]},];


let ENV_OBJECTS  = [    {"plane":[15, 0, 0]},
                    {"rock":[290, 0, 50],"scale":1.5},
                    {"rock": [140,0,-30],"scale":2},
                    {"rock":[60, 0, -30],"scale":1.3},
                    {"rock":[30, 0, -60],"scale":1},
                    {"rock":[140, 0, -120],"scale":2.5},
                    {"rock":[280, 0, 120],"scale":1.5},
                    {"rock":[180, 0, 160],"scale":1.5},
                    {"cactus":[100, 0, 0],"scale":1.5},
                    {"cactus":[40, 0, 70],"scale":2},
                    {"cactus":[80, 0, -70],"scale":1.3},
                    {"cactus":[100, 0, 50],"scale":1.4},
                    {"cactus":[240, 0, 140],"scale":2},
                    {"cactus":[320, 0, 170],"scale":1.3},
                    {"cactus":[300, 0, 20],"scale":1.5},
                    {"rock":[290, 0, 50],"scale":1.5},
                    {"rock":[60, 0, -30],"scale":1.3},
                    {"cactus":[100, 0, 0],"scale":1.5},
                    {"cactus":[60, 0, 0],"scale":2},
                    {"rock" : [50,0,20],"scale":3},
                    {"cactus" : [60,0,60],"scale":2},
                    {"rock" : [50,0,-50],"scale":2},
                    {"cactus" : [60,0,-90],"scale":1.5},
                    {"cactus" : [50,0,-100],"scale":1.5},
                    {"rock" : [0,0,-80],"scale":3},
                    {"rock": [250,0,-120],"scale":7},
                    {"cactus": [200,0,-150],"scale":3},
                    {"rock": [40,0,-160],"scale":5},
                    {"cactus": [170,0,-0],"scale":3},
                    {"rock": [40,0,100],"scale":3},
                    {"cactus": [30,0,40],"scale":3}, 
                    {"cactus": [180,0,140],"scale":3},
                    {"cactus": [180,0,120],"scale":3}, 
                    {"rock": [200,0,140],"scale":1.5},
                    {"cactus": [180,0,-20],"scale":2},
                    {"cactus": [170,0,60],"scale": 2.5},
                    {"cactus":[300,0,-50],"scale": 2},
                    {"rock": [90,0,140],"scale": 6},
                    {"cactus": [50,0,-10],"scale": 1},
                    {"cactus": [150,0,-80],"scale": 2}];



const LEVEL1 = [CONFIG,ENV_OBJECTS, TARGETS];




ENV_OBJECTS = [ {"plane":[15, 0, 0]},
                    {"tree":[310, 0, 80],"scale":1.2},
                    {"tree":[80, 0, 80],"scale":1.3},
                    {"lake":[200, 1, 80]},
                    {"rock":[290, 0, 50],"scale":1.5},
                    {"tree":[82, 0, 10],"scale":1.4},
                    {"tree":[100, 0, -30],"scale":0.8},
                    {"tree":[200, 0, -30],"scale":1.5},
                    {"rock": [140,0,-30],"scale":2},
                    {"tree":[310, 0, -30],"scale":1.1},
                    {"rock":[60, 0, -30],"scale":1.3},
                    {"tree":[30, 0, -80],"scale":1.1},
                    {"rock":[30, 0, -60],"scale":1},
                    {"tree":[150, 0, -90],"scale":1.2},
                    {"tree":[100, 0, -120],"scale":1.3},
                    {"rock":[140, 0, -120],"scale":2.5},
                    {"bush":[100, 0, 0],"scale":1.5},  
                    {"bush":[40, 0, 70],"scale":1.2},
                    {"bush":[80, 0, -70],"scale":1.3},
                    {"bush":[100, 0, 50],"scale":1.4},
                    {"tree":[300, 0, 120],"scale":1.3},
                    {"rock":[280, 0, 120],"scale":1.5},
                    {"tree":[200, 0, 160],"scale":1.2},
                    {"rock":[180, 0, 160],"scale":1.5},
                    {"bush":[240, 0, 140],"scale":2},
                    {"tree":[320, 0, 170],"scale":1.3},
                    {"rock": [120,0,-80],"scale":2},
                    {"bush":[150,0,-60],"scale":2.5},
                    {"tree":[200,0,-70],"scale":1.5},
                    {"bush": [80,0,120],"scale":3},
                    {"tree":[30,0,150],"scale":1.5},
                    {"rock":[40,0,115],"scale":2},
                    {"rock": [100,0,150],"scale":2.5},
                    {"rock": [80,0,60], "scale": 1.5},
                    {"bush": [80,0,30],"scale":2.3},
                    {"rock":[90,0,-15],"scale":1.7},
                    {"tree": [25,0,90],"scale":0.8},
                    {"bush": [250,0,-130],"scale":2},
                    {"rock": [220,0,-160],"scale":3},
                    {"rock": [280,0,-0], "scale": 4},
                    {"bush": [10,0,-100],"scale":2},];

TARGETS= [{"duck":[50,30, 0]},
                {"fox":[120, 0, 50]},
                {"boar":[300, 0, -150]},
                {"duck":[40, 30, 170]},];



CONFIG = [{"bullets": 6}]

const LEVEL2 = [CONFIG,ENV_OBJECTS, TARGETS];

ENV_OBJECTS = [ {"plane":[15, 0, 0]},
                    {"tree":[310, 0, 80],"scale":1.2},
                    {"tree":[80, 0, 80],"scale":1.3},
                    {"rock":[290, 0, 50],"scale":1.5},
                    {"tree":[82, 0, 10],"scale":1.4},
                    {"tree":[100, 0, -30],"scale":0.8},
                    {"tree":[200, 0, -30],"scale":1.5},
                    {"rock": [140,0,-30],"scale":2},
                    {"tree":[310, 0, -30],"scale":1.1},
                    {"rock":[60, 0, -30],"scale":1.3},
                    {"tree":[30, 0, -80],"scale":1.1},
                    {"rock":[30, 0, -60],"scale":1},
                    {"tree":[150, 0, -90],"scale":1.2},
                    {"tree":[100, 0, -120],"scale":1.3},
                    {"rock":[140, 0, -120],"scale":2.5},
                    {"bush":[100, 0, 0],"scale":1.5},  
                    {"bush":[40, 0, 70],"scale":1.2},
                    {"bush":[80, 0, -70],"scale":1.3},
                    {"bush":[100, 0, 50],"scale":1.4},
                    {"tree":[300, 0, 120],"scale":1.3},
                    {"rock":[280, 0, 120],"scale":1.5},
                    {"tree":[200, 0, 160],"scale":1.2},
                    {"rock":[180, 0, 160],"scale":1.5},
                    {"bush":[240, 0, 140],"scale":2},
                    {"tree":[320, 0, 170],"scale":1.3},
                    {"rock": [120,0,-80],"scale":2},
                    {"bush":[150,0,-60],"scale":2.5},
                    {"tree":[200,0,-70],"scale":1.5},
                    {"bush": [80,0,120],"scale":3},
                    {"tree":[30,0,150],"scale":1.5},
                    {"rock":[40,0,115],"scale":2},
                    {"rock": [100,0,150],"scale":2.5},
                    {"rock": [80,0,60], "scale": 1.5},
                    {"bush": [80,0,30],"scale":2.3},
                    {"rock":[90,0,-15],"scale":1.7},
                    {"tree": [25,0,90],"scale":0.8},
                    {"bush": [250,0,-130],"scale":2},
                    {"rock": [220,0,-160],"scale":3},
                    {"rock": [280,0,-0], "scale": 4},
                    {"bush": [10,0,-100],"scale":2},
                    {"tree":[200,0,70],"scale":1.5},
                    {"bush": [200,0,40],"scale":2.5}];



TARGETS = TARGETS= [
                    {"fox":[120, 0, 50]},
                    {"boar":[300, 0, -150]},
                    {"fox":[270, 0, 120]},];



const LEVEL3 = [CONFIG,ENV_OBJECTS, TARGETS];



 ENV_OBJECTS  = [    {"plane":[15, 0, 0]},
                    {"rock":[290, 0, 50],"scale":1.5},
                    {"rock": [140,0,-30],"scale":2},
                    {"rock":[60, 0, -30],"scale":1.3},
                    {"rock":[30, 0, -60],"scale":1},
                    {"rock":[140, 0, -120],"scale":2.5},
                    {"rock":[280, 0, 120],"scale":1.5},
                    {"rock":[180, 0, 160],"scale":1.5},
                    {"cactus":[40, 0, 70],"scale":2},
                    {"cactus":[80, 0, -70],"scale":1.3},
                    {"cactus":[100, 0, 50],"scale":1.4},
                    {"cactus":[240, 0, 140],"scale":2},
                    {"cactus":[320, 0, 170],"scale":1.3},
                    {"cactus":[300, 0, 20],"scale":1.5},
                    {"rock":[290, 0, 50],"scale":1.5},
                    {"rock":[60, 0, -30],"scale":1.3},
                    {"rock" : [50,0,20],"scale":2},
                    {"cactus" : [60,0,60],"scale":2},
                    {"rock" : [50,0,-50],"scale":2},
                    {"cactus" : [60,0,-90],"scale":1.5},
                    {"cactus" : [50,0,-100],"scale":1.5},
                    {"rock" : [0,0,-80],"scale":3},
                    {"rock": [250,0,-120],"scale":7},
                    {"cactus": [200,0,-150],"scale":3},
                    {"rock": [40,0,-160],"scale":5},
                    {"rock": [40,0,100],"scale":3},
                    {"cactus": [30,0,40],"scale":3}, 
                    {"cactus": [180,0,140],"scale":3},
                    {"cactus": [180,0,120],"scale":3}, 
                    {"rock": [200,0,140],"scale":1.5},
                    {"cactus": [180,0,-20],"scale":2},
                    {"cactus": [170,0,60],"scale": 2.5},
                    {"cactus":[300,0,-50],"scale": 2},
                    {"rock": [90,0,140],"scale": 6},
                    {"cactus": [50,0,-10],"scale": 1},
                    {"cactus": [150,0,-80],"scale": 2}];



TARGETS = [{"enemy": [100,0,0]},
                 {"enemy": [200,0,120],"rotation": 0.82 * Math.PI},
                 {"enemy": [120,0,-40],"rotation": 3.3 *Math.PI/3},
                 {"enemy": [150,0,30],"rotation":Math.PI - 0.2},
                 {"enemy": [70,0,-100],"rotation":4 *Math.PI/3},
                 {"enemy": [20,0,150],"rotation":Math.PI/2 + 0.1},
            

];


CONFIG = [{"bullets": 8}]




const LEVEL4 = [CONFIG,ENV_OBJECTS, TARGETS];


export const MAP = [LEVEL1,LEVEL2,LEVEL3,LEVEL4];

